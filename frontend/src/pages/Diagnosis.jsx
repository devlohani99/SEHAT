...
const HealthcareAIApp = () => {
    ...
    const [scanType, setScanType] = useState('chest_xray'); // Default scan type

    // Comprehensive disease database organized by scan type
    ...
    const analyzeMedicalImage = async () => {
        ...
        try {
            ...
            // Get diseases for the selected scan type
            const scanDiseases = diseaseDatabase[scanType] || {};
            const diseaseKeys = Object.keys(scanDiseases);

            // Generate mock responses based on scan type
            const mockResponses = diseaseKeys.map(disease => ({
                disease,
                confidence: Math.random() * 0.8 + 0.1 // 10-90% confidence
            }));

            // Add normal result with lower probability
            mockResponses.push({
                disease: "normal",
                confidence: Math.random() * 0.3 // 0-30% confidence
            });

            // Filter to only show predictions with >15% confidence
            const validPredictions = mockResponses.filter(d => d.confidence > 0.15);

            // Sort by confidence and take the top prediction
